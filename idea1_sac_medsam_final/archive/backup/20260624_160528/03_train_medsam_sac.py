    ...
    for epoch in range(args.epochs):
        ...
        for entry in schedule:
            ...
            if entry["label_mode"] == "full":
                ...
                # WAC: weak activation calibration
                weak1 = ((p_low > 0.30) & (p_low < 0.40) & (box_mask_low > 0.5)).float()
                weak2 = ((p_low >= 0.40) & (p_low < 0.50) & (box_mask_low > 0.5)).float()

                strong = ((p_ema_low >= 0.50) & (qf_low > 0.55) & (box_mask_low > 0.5)).float()

                # local strong target by max-pooling. Kernel 31 approximates r=15.
                # A larger radius is used because weak activations often lie around the boundary
                # and may be farther from the reliable strong core in low-resolution mask space.
                local_target = F.max_pool2d(p_ema_low * strong, kernel_size=31, stride=1, padding=15)
                local_presence = F.max_pool2d(strong, kernel_size=31, stride=1, padding=15)

                shape_reliability = (1.0 - 4.0 * shape_low * (1.0 - shape_low)).clamp(0, 1)
                # Use shape prior as a soft reliability weight rather than a hard spatial exclusion.
                shape_gate = (0.50 + 0.50 * shape_reliability).clamp(0.50, 1.0)
                qf_gate = (0.25 + 0.75 * qf_low).clamp(0.25, 1.0)
                gate_base = ((local_presence > 0.50).float() * shape_gate * qf_gate)
                ...
                if gate1.sum() > 0:
                    ...
                    loss_wac = loss_wac + (wac_map * gate1).sum() / gate1.sum().clamp_min(1.0)
                ...
                if gate2.sum() > 0:
                    ...
                    loss_wac = loss_wac + 0.5 * (wac_map * gate2).sum() / gate2.sum().clamp_min(1.0)
